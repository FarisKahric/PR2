...
public:
    ...
    Parovi operator()(int from, int to, bool obrni = false) {
        Parovi op;
...
public:
    ...
    void DodajKomentarKarakteristike(Karakteristike k, const char* sadrzaj) {
        for (size_t i = 0; i < _komentariKarakteristika->getTrenutno(); i++)
        {
            if (*_komentariKarakteristika->getElement1(i) == k)
                throw exception("Karakteristika vec dodana. \n");
        }
        _komentariKarakteristika->AddElement(new Karakteristike(k), sadrzaj);
    }
...
public:
    ...
    // virtual kod nasljedjivanja mora ici
    virtual~Osoba() {
        delete[] _imePrezime; _imePrezime = nullptr;
    }
...
public:
    //dodati nasljedjivanje konstruktora iz osobe
