...
public:
	...
	friend bool operator== (const Osoba& o1, const Osoba& o2) {
		return (strcmp(o1._imePrezime, o2._imePrezime) == 0 && o1._datumRodjenja == o2._datumRodjenja);
	}
...
class Kupac : public Osoba {
	...
	void PosaljiMail(int bodovi) {
		...
		cout << endl;
		cout << "Zavhaljujemo vam na kupovini." << endl;
...
public:
	//nasljedjivanje
	Kupac(const char* imePrezime = "", Datum datumRodjenja = Datum(), const char* emailAdresa = "") : Osoba(imePrezime, datumRodjenja) { //nasljedjivanje 
	...
	void DodajKupovinu(int novac, ZadovoljstvoKupca zad) {
		if (novac > 10)
		...
		_kupovine->AddElement(novac, zad);
	}
	...
	Rjecnik<Karakteristike, const char*> GetKupovineByKomentar(const char* sadrzaj) {
		...
		for (size_t i = 0; i < _kupovine->getTrenutno(); i++)
		{
			...
			{
				if (strstr(_kupovine->getElement2(i).GetKomentareKarakteristika().getElement2(j), sadrzaj) != nullptr) {
